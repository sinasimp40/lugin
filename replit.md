...
- **public/index.html** — Login UI + session view + insert coin modal + admin modal (580px wide, single page); scramble text computer name, auto-shutdown countdown with horizontal progress bar, secret "zxc1" admin trigger; black & orange theme; login auto-prepends `mem-` prefix; registration validates no special chars/spaces; ad carousel slider between login form and shutdown timer with configurable interval (1-60s), dot navigation, image+rich-text slides
