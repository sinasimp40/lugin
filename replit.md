...
- **src/settings-store.js** — JSON file settings storage in `./data/`; scrypt password hashing; manages computer name, auto-shutdown timer, background image metadata, login panel image metadata, advertisements (slides with images + rich HTML content); HTML sanitizer allows formatting, links, images, lists, blockquotes, headings
